...
_Цена, которую вы платите_: **Инфраструктурный Ад**.
...
_Совет_: Если вы стартап или небольшая команда — **стройте Монолит**. Хороший, модульный монолит. Не занимайтесь "Resume Driven Development".

## Новые горизонты

### Serverless: Код без Серверов

Мы прошли путь: Железо → Виртуальные Машины → Контейнеры.
Следующий шаг — **Serverless** (AWS Lambda, Google Cloud Functions).
Идея проста: вы вообще не хотите знать, что такое "сервер", "Linux" или "Docker". Вы просто загружаете ZIP-архив с кодом функции (например, "Изменить размер картинки") и говорите облаку: "Запускай это каждый раз, когда кто-то грузит фото".
Вы платите только за миллисекунды работы процессора. Если фото никто не грузит — вы платите **ноль**. Это идеально для событийных задач, но пока сложно для больших приложений.

### DevSecOps: Безопасность не для галочки

Традиционно безопасники (Security) были "Доктором Нет". Они приходили перед релизом, находили 100 уязвимостей и блокировали деплой на месяц.
DevSecOps провозглашает: **Безопасность должна быть частью конвейера**.

1.  **SAST (Static Analysis)**: Проверка кода на уязвимости прямо в Git (до коммита).
2.  **Container Scanning**: Автоматическая проверка Docker-образов на дыры.
3.  Если вы нашли дыру, конвейер падает сам, не доходя до продакшена.
...
- _Плохой API_: `POST /pay` (снимает $10). Если нажать дважды (у пользователя залагал интернет) — спишется $20.
- _Хороший API_: `POST /pay?request_id=uuid`. Если нажать дважды с тем же ID, сервер скажет: "Друг, я уже обработал этот платеж, всё ок".
...
- **[The Phoenix Project](https://itrevolution.com/book/the-phoenix-project/)** — Роман про DevOps. Читается как детектив. Если хотите понять, _зачем_ мы всё это делаем — начните отсюда.
