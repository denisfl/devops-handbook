...
1.  **Vertical Scaling (Scale Up)**: Купить сервер помощнее. Больше ядер, больше памяти.
    *Проблема*: Это дорого, и есть физический потолок. Вы не можете поставить 1000 терабайт памяти в одну машину.
2.  **Horizontal Scaling (Scale Out)**: Поставить рядом еще 5 таких же дешевых серверов.
    *Проблема*: Пользователь знает только один адрес `myshop.com`. Как распределить запросы?
