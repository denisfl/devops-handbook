# День 5. Python для автоматизации

[tools] [devops] [python] [automation]

Любой системный администратор начинает свой путь с Bash. Это родной язык терминала, идеальный для простых задач: "скопируй файлы", "запусти сервер", "удали старые логи".
Но в какой-то момент Bash превращается в ловушку. Вы пытаетесь распарсить JSON-ответ от сервера, используя `grep`, `sed` и `awk`. Ваш скрипт обрастает костылями, становится нечитаемым и хрупким.
В этот момент на сцену выходит **Python**. Это "клей", который соединяет разрозненные системы в единую автоматизированную платформу.

---

## Почему Python стал стандартом в DevOps?

Если Bash — это молоток, то Python — это станок с ЧПУ.

1.  **Читаемость**. Код на Python часто читается как английский текст. В Ops-командах, где инженеры часто сменяют друг друга, это критично.
2.  **Структуры данных**. Bash оперирует только строками. Python дает нам Списки и Словари. А так как современный мир (AWS, Kubernetes, REST API) разговаривает на языке JSON, Python с его словарями (`dict`) становится нативным инструментом.
3.  **Экосистема**. Нужно отправить уведомление в Telegram? `import telebot`. Нужно получить данные из SQL базы? `import psycopg2`. Вам не нужно писать велосипеды.

### Анатомия скрипта

Инженерный скрипт обычно строится по шаблону:

1.  **Imports**: Подключаем батарейки (`os`, `sys`, `requests`).
2.  **Config**: Выносим константы (URL, токены) в начало или читаем из переменных окружения.
...
# Конструкция 'with' гарантирует закрытие файла
...
# Публичное API для определения IP
