...
Чтобы понять Linux, нужно понять философию Unix, заложенную полвека назад.

### Анатомия: Kernel vs User Space
Linux — это не монолитный кусок кода. Глобально память компьютера и привилегии выполнения делятся на два уровня:
1.  **Kernel Space (Пространство Ядра)**. Привилегированный режим. Здесь работает Ядро (Kernel), имеющее прямой и неограниченный доступ к аппаратному обеспечению (CPU, Память, Диски). Ошибка на этом уровне критична и приводит к остановке всей системы (Kernel Panic).
2.  **User Space (Пространство Пользователя)**. Ограниченный режим. Здесь выполняются прикладные программы: браузеры, редакторы, оболочка Bash. Они не имеют прямого доступа к памяти других процессов или оборудованию.

**Взаимодействие уровней: System Calls**
Когда вы в Bash пишете `ls`, программа не обращается к диску напрямую. Она не имеет на это прав. Вместо этого она отправляет запрос Ядру: "Выполни системный вызов `getdents64`". Ядро проверяет права доступа (ACL), считывает данные с контроллера диска и возвращает результат в User Space.
Понимание этой границы важно для диагностики: находится ли проблема в логике приложения (User Space) или в исчерпании системных ресурсов/лимитов (Kernel Space).

### Принципы Unix-way
1.  **Всё есть файл**. Это абстракция. Жесткий диск — это файл (`/dev/sda`). Клавиатура — файл. Сетевой сокет — файл.
    *Почему это гениально?* Это унифицирует API. Программисту не нужно учить 10 разных команд для чтения с диска, чтения с флешки или чтения из сети. Он использует один `read()`. Все файлы в Linux идентифицируются числом — **File Descriptor (FD)**.
2.  **Делайте одну вещь, и делайте её хорошо**. В отличие от Windows, где программы часто представляют собой "комбайны", в Linux принято создавать маленькие утилиты. `ls` только показывает файлы. `grep` только ищет текст. Сила системы проявляется в возможности комбинировать эти утилиты через **Пайпы** (`|`), передавая стандартный вывод (`stdout`) одной программы на стандартный вход (`stdin`) другой.
