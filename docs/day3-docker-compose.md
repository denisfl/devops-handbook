...
Образ Docker (Image) технически представляет собой набор неизменяемых слоев (RO - Read Only). Каждая инструкция в `Dockerfile` (например, `RUN apt-get install python`) создает новый слой, содержащий только разницу (delta) по сравнению с предыдущим.

- Слой 1: Базовая ОС (RootFS).
- Слой 2: Библиотеки и зависимости.
- Слой 3: Код приложения.

**Кэширование сборки**: Это критически важный механизм оптимизации. Если вы изменили код (Слой 3), Docker обнаружит, что хэши предыдущих слоев не изменились, и переиспользует их из кэша. Пересборка коснется только измененного слоя.

Виртуальная файловая система (OverlayFS или UnionFS) объединяет эти слои в единое представление.
При запуске контейнера Docker добавляет сверху тонкий слой **для записи (RW слой)**. Изменения, вносимые процессом во время работы, сохраняются только в этом верхнем слое. Это реализует механизм **Copy-on-Write (CoW)**: пока файл не изменен, все контейнеры читают его единственную копию из образа. При попытке изменения файл копируется в верхний слой.
Это позволяет запускать сотни идентичных контейнеров с минимальными накладными расходами на дисковое пространство.
